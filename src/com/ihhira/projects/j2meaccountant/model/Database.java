...
import com.ihhira.projects.j2meaccountant.Log;
...
//        try {
//            pm.getProperty(Settings.KEY_THEME_INDEX);
//        } catch (Exception ex) {
//            ex.printStackTrace();
//            pm.setProperty(Settings.KEY_THEME_INDEX, new Integer(0));
//        }
    }
    ...
    public static Vector getAccounts() {
        ...
        try {
            ...
            ObjectSet taccounts = pm.find(Account.class, new Filter() {
                public boolean matches(Persistable prstbl) {
                    return true;
                }
            }, new Comparator() {
                public int compare(Persistable prstbl, Persistable prstbl1) {
                    return prstbl.hashCode() - prstbl1.hashCode();
                }
            });
            Log.log("taccounts: " + taccounts);
            ...
            Log.log("accounts listed " + accounts.size());
    ...
    public static double getAccountBalance(final Account account) {
    ...
    public static Vector getTransactions(final Account account) {
        ...
        try {
            ObjectSet transactions = pm.find(Transaction.class, new Filter() {
                ...
                public int compare(Persistable prstbl, Persistable prstbl1) {
                    if (((Transaction) prstbl).date.getTime() < ((Transaction) prstbl1).date.getTime()) {
                        return FOLLOWS;
                    }
                    return PRECEDES;
                }
